...
if isempty(cover_coords)
    ...
    sx_cover = findsx_direct(xs_quads, cover_coords, par.Rc, par.intp_func);  
end

sx_cover.nodes_in_sx = sx_cover.nodes_in_sx + par.dv_num;
sx_default = findsx_direct(xs_quads, dv_coords, par.Rc, par.intp_func, sx_cover);

tp = findsx_direct(dv_coords, density_coords, par.Rc_pseudograd, par.intp_func);
...
for i=1:par.dv_num
    ...
    dist_tp = tp.dist([1:i_pos-1, i_pos+1:num_i], i);
    f_tp = interp_function(dist_tp, par.intp_func, 0);
    df_tp = interp_function(dist_tp, par.intp_func, 1);
    
    sumf_tp = sum(f_tp);
    coeff(1:num_i-1,i) = df_tp./dist_tp / sumf_tp;
